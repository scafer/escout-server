...
public class AuthenticationController : ControllerBase
{
    ...
    public ActionResult<AuthData> SignIn(User user)
    {
        ...
        if (account == null) return BadRequest(ConstValues.MSG_ACCOUNT_NOT_FOUND);

        if (account.accessLevel.Equals(-1)) return Forbid(ConstValues.MSG_ACCOUNT_UNAUTHORIZED);

        if (TokenService.VerifyPassword(user.password, account.password)) return TokenService.GenerateToken(account);
    ...
    {
        if (CheckEmailExist(user.email)) return BadRequest(ConstValues.MSG_EMAIL_USED);

        if (CheckUsernameExist(user.username)) return BadRequest(ConstValues.MSG_USERNAME_USED);
        ...
        try
        {
            ...
            if (user.notifications == 1)
                _ = Notifications.SendEmail(user.email, ConstValues.NTF_TITLE_GENERIC,
                    string.Format(ConstValues.NTF_BODY_NEW_ACCOUNT, user.username));
    ...
    public IActionResult ResetPassword(User user)
    {
        ...
        if (userData == null) return BadRequest(ConstValues.MSG_ACCOUNT_NOT_FOUND);
    ...
    {
        return dataContext.users.FirstOrDefault(u => u.email == email) != null;
    }

    private bool CheckUsernameExist(string username)
    {
        return dataContext.users.FirstOrDefault(u => u.username == username) != null;
    }
}