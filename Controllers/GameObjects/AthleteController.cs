...
public class AthleteController : ControllerBase
{
    ...
    public ActionResult<List<Athlete>> GetAthletes(string query)
    {
        try
        {
            ...
            else
            {
                ...
                var q = string.Format(ConstValues.QUERY, "athletes", criteria.fieldName, criteria.condition,
                    criteria.value);
                athletes = dataContext.athletes.FromSqlRaw(q).ToList();
            }

            foreach (var athlete in athletes) athlete.displayOptions = GetAthleteDisplayOptions(athlete);
    ...
    private Dictionary<string, string> GetAthleteDisplayOptions(Athlete athlete)
    {
        ...
        {
            var club = dataContext.clubs.FirstOrDefault(a => a.id == athlete.clubId);
        ...
        {
            var image = dataContext.images.FirstOrDefault(a => a.id == athlete.imageId);
