...
public class ClubController : ControllerBase
{
    ...
    public ActionResult<List<Club>> GetClubs(string query)
    {
        try
        {
            ...
            else
            {
                ...
                var q = string.Format(ConstValues.QUERY, "clubs", criteria.fieldName, criteria.condition,
                    criteria.value);
                clubs = dataContext.clubs.FromSqlRaw(q).ToList();
            }

            foreach (var club in clubs) club.displayOptions = GetClubDisplayOptions(club);
    ...
    private Dictionary<string, string> GetClubDisplayOptions(Club club)
    {
        ...
        {
            var image = dataContext.images.FirstOrDefault(a => a.id == club.imageId);
