...
public class GameController : ControllerBase
{
    ...
    public ActionResult<List<Game>> CreateGame(List<Game> game)
    {
        ...
        if (user.accessLevel >= 3) return Forbid();
    ...
    {
        if (User.GetUser(dataContext).accessLevel >= 3) return Forbid();
    ...
    public ActionResult<List<Game>> GetGames(string query)
    {
        try
        {
            ...
            else
            {
                ...
                var q = string.Format(ConstValues.QUERY, "games", criteria.fieldName, criteria.condition,
                    criteria.value);
    ...
    public ActionResult<List<GameEvent>> GetGameEvents(int gameId)
    {
        ...
        foreach (var gameEvent in gameEvents) gameEvent.displayOptions = GetGameEventDisplayOptions(gameEvent);
    ...
    public ActionResult<List<GameUser>> GetGameUser(string query)
    {
        try
        {
            ...
            {
                var criteria = JsonConvert.DeserializeObject<FilterCriteria>(query);
                var q = string.Format(ConstValues.QUERY, "gameUsers", criteria.fieldName, criteria.condition,
                    criteria.value);
    ...
    public ActionResult<List<GameUser>> CreateGameUser(List<GameUser> gameUsers)
    {
        ...
        {
            var obj = dataContext.gameUsers.Where(c =>
                    c.gameId == gameUser.gameId && c.userId == gameUser.userId && c.athleteId == gameUser.athleteId)
                .ToList();
            if (obj.Count != 0) gameUsers.Remove(gameUser);
        }
    ...
    {
        return dataContext.gameEvents.Where(g => g.athleteId == athleteId && g.gameId == gameId).OrderBy(x => x.id)
            .ToList();
    }
    ...
    public ActionResult<GameData> GetGameData(int gameId)
    {
        ...
        gameData.clubs = dataContext.clubs.Where(t => t.id == gameData.game.homeId || t.id == gameData.game.visitorId)
            .ToList();
        gameData.athletes = dataContext.athletes
            .Where(t => t.clubId == gameData.game.homeId || t.clubId == gameData.game.visitorId).ToList();
    ...
    private int GetGameStatus(Game game)
    {
        try
        {
            ...
            if (DateTime.Compare(actualTime, gameStart) < 0) return ConstValues.GS_PENDING;

            if (DateTime.Compare(actualTime, gameEnd) > 0) return ConstValues.GS_FINISHED;

            if (DateTime.Compare(actualTime, gameStart) >= 0 && DateTime.Compare(actualTime, gameEnd) <= 0)
    ...
    private Dictionary<string, string> GetGameDisplayOptions(Game game)
    {
        ...
        {
            var image = dataContext.images.FirstOrDefault(a => a.id == game.imageId);
        ...
        {
            var competition = dataContext.competitions.FirstOrDefault(a => a.id == game.competitionId);

            if (competition != null) displayOptions.Add(ConstValues.DO_COMPETITION_NAME, competition.name);
        }

        if (game.userId != 0)
        {
            var user = dataContext.users.FirstOrDefault(a => a.id == game.userId);

            if (user != null) displayOptions.Add(ConstValues.DO_USER_NAME, user.username);
        }
        ...
        if (game.homeId != 0)
        {
            ...
            if (homeClub != null) displayOptions.Add(ConstValues.DO_HOME_CLUB_NAME, homeClub.name);

            if (homeClub.imageId != null)
            {
                var image = dataContext.images.FirstOrDefault(a => a.id == homeClub.imageId);

                if (image != null) displayOptions.Add(ConstValues.DO_HOME_IMAGE_URL, image.imageUrl);
            }
        }
        ...
        if (game.visitorId != 0)
        {
            ...
            if (visitorClub != null) displayOptions.Add(ConstValues.DO_VISITOR_CLUB_NAME, visitorClub.name);

            if (visitorClub.imageId != null)
            {
                var image = dataContext.images.FirstOrDefault(a => a.id == visitorClub.imageId);

                if (image != null) displayOptions.Add(ConstValues.DO_VISITOR_IMAGE_URL, image.imageUrl);
            }
        }
    ...
    private Dictionary<string, string> GetGameEventDisplayOptions(GameEvent gameEvent)
    {
        ...
        {
            var athlete = dataContext.athletes.FirstOrDefault(a => a.id == gameEvent.athleteId);

            if (athlete != null) displayOptions.Add(ConstValues.DO_ATHLETE_NAME, athlete.name);
        }

        if (gameEvent.clubId != null)
        {
            var club = dataContext.athletes.FirstOrDefault(a => a.id == gameEvent.clubId);

            if (club != null) displayOptions.Add(ConstValues.DO_CLUB_NAME, club.name);
        }

        if (gameEvent.eventId != 0)
        {
            var evt = dataContext.events.FirstOrDefault(a => a.id == gameEvent.eventId);

            if (evt != null) displayOptions.Add(ConstValues.DO_EVENT_NAME, evt.name);
        }
