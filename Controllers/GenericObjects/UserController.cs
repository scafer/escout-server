...
public class UserController : ControllerBase
{
    ...
    public ActionResult<List<User>> GetUsers(string query)
    {
        if (User.GetUser(dataContext).accessLevel == ConstValues.AL_ADMINISTRATOR)
        {
            if (string.IsNullOrEmpty(query))
            {
                ...
                foreach (var user in allUsers) user.displayOptions = GetUserDisplayOptions(user);
            ...
            foreach (var user in filteredUsers) user.displayOptions = GetUserDisplayOptions(user);
    ...
    private Dictionary<string, string> GetUserDisplayOptions(User user)
    {
        ...
        {
            var image = dataContext.images.FirstOrDefault(a => a.id == user.imageId);
